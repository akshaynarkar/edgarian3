...
# Parent rollup labels — drop before segment set scoring
_ROLLUP_KEYWORDS = {
    "operating segments", "products", "total", "consolidated", "all other",
    "corporate", "eliminations", "segment total", "reportable segments",
}
...
def _is_rollup(name: str) -> bool:
    n = name.lower().strip()
    return any(kw == n or n.startswith(kw) for kw in _ROLLUP_KEYWORDS)


def _select_best_segment_set(segments: list[Segment], revenue: float) -> list[Segment]:
    if not segments:
        return []
    segments = [s for s in segments if not _is_rollup(s.segment_name)]
    if not segments:
        return []
...
function syncTheme() {{
  try {{
    ...
    if (p) {{
      document.body.classList.toggle('light', p.classList.contains('light'));
      return true;
    }}
  }} catch(e) {{}}
