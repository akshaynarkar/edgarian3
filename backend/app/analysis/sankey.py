...
def build_sankey_payload(symbol: str, db: Session) -> dict[str, Any]:
    ...
    if financial is None:
        return {
            ...
            "title": f"{ticker} Income Statement", "period": None,
    ...
    for segment in segments:
        ...
        if seg_value <= 0:
            continue
        nodes.append({"id": next_id, "label": segment.segment_name, "value": seg_value, "type": "revenue_segment"})
        revenue_segment_ids.append(next_id)
        next_id += 1

    revenue_id = next_id
    nodes.append({"id": revenue_id, "label": "Revenue", "value": revenue, "type": "revenue"})
    next_id += 1
    gross_profit_id = next_id
    nodes.append({"id": gross_profit_id, "label": "Gross Profit", "value": gross_profit, "type": "profit"})
    next_id += 1
    cost_of_revenue_id = next_id
    nodes.append({"id": cost_of_revenue_id, "label": "Cost of Revenue", "value": cost_of_revenue, "type": "cost"})
    next_id += 1
    operating_income_id = next_id
    nodes.append({"id": operating_income_id, "label": "Operating Income", "value": operating_income, "type": "profit"})
    next_id += 1
    net_income_id = next_id
    nodes.append({"id": net_income_id, "label": "Net Income", "value": net_income, "type": "profit"})
    next_id += 1
    income_tax_id = next_id
    nodes.append({"id": income_tax_id, "label": "Income Tax", "value": income_tax, "type": "cost"})
    next_id += 1
    other_net_id = next_id
    nodes.append({"id": other_net_id, "label": "Other (net)", "value": other_net, "type": "other"})

    for segment_id in revenue_segment_ids:
        node_value = next(n["value"] for n in nodes if n["id"] == segment_id)
    ...
    payload = {
        ...
        "currency": "USD",
        "unit": "B",
        "nodes": [{"id": n["id"], "label": n["label"], "value": abs(n["value"]), "type": n["type"]} for n in nodes],
...
<head>
  ...
  <style>
    /* Dark defaults */
    :root {{
      --bg: #0a0a0a; --bg2: #111111; --text: #f0ede8; --muted: #7a7874;
      --accent: #c8c0b0; --link-color: rgba(255,255,255,0.18);
    }}
    /* Light mode override — detected from parent or media query */
    :root.light {{
      --bg: #f4f2ee; --bg2: #ffffff; --text: #18170f; --muted: #6b6760;
      --accent: #2c2a22; --link-color: rgba(0,0,0,0.10);
    }}
    html, body {{ margin: 0; padding: 0; background: var(--bg); color: var(--text); font-family: Helvetica Neue, Arial, sans-serif; }}
    .wrap {{ background: var(--bg2); border-radius: 8px; padding: 14px; }}
    .header {{ display: flex; align-items: center; gap: 12px; margin-bottom: 12px; }}
    .header img {{ width: 24px; height: 24px; object-fit: contain; border-radius: 4px; }}
    .title {{ font-family: Georgia, serif; font-size: 20px; font-weight: 700; color: var(--text); line-height: 1.1; }}
    .sub {{ font-size: 11px; letter-spacing: 0.18em; text-transform: uppercase; color: var(--muted); margin-top: 3px; }}
...
// Sync theme from parent app — detect light/dark and apply to iframe root
function syncTheme() {{
  try {{
    const parentApp = window.parent.document.getElementById('app');
    const isLight = parentApp ? parentApp.classList.contains('light') : false;
    if (isLight) {{
    ...
    // cross-origin fallback: use media query
...
// Node colors — hardcoded hex so they work in both modes
function nodeColor(type) {{
  if (type === 'revenue' || type === 'revenue_segment') return '#c8c0b0';
  if (type === 'profit') return '#97c459';
  if (type === 'cost') return '#f09595';
  return '#7a7874';
}}

function fmtVal(v) {{
  return '$' + (v / 1e9).toFixed(1) + payload.unit;
}}
...
document.getElementById('sub').textContent =
  (payload.period || 'Latest') + ' \u00b7 ' + payload.currency + ' \u00b7 UNIT ' + payload.unit;

// Detect light for plotly background/font
const isLight = document.documentElement.classList.contains('light');
const plotBg = isLight ? '#ffffff' : '#111111';
const fontColor = isLight ? '#18170f' : '#f0ede8';
const linkColor = isLight ? 'rgba(0,0,0,0.10)' : 'rgba(255,255,255,0.18)';
...
const data = [{{
  ...
  node: {{
    pad: 18,
    thickness: 16,
    label: payload.nodes.map((n) => n.label + ' (' + fmtVal(n.value) + ')'),
  ...
  link: {{
    ...
    color: payload.links.map(() => linkColor)
  }}
}}];
...
const layout = {{
  ...
  font: {{ family: 'Helvetica Neue, Arial, sans-serif', size: 12, color: fontColor }}
}};
